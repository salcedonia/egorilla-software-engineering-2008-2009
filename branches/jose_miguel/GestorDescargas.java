...
/**
 *
 * @author Luis Ayuso, Jose Miguel Guerrero
 */
public class GestorDescargas {
    ...
    public void llegaFragmento(Fragmento f, Byte[] chunk){
        ...
        if (!_temporales.containsKey(f._hash))
            return;
        
        // esta este fragmento entre los que no tenemos?
        
        boolean encontrado = false;
        for (Fragmento frag : _temporales.get(f._hash).getFragmentos()) {
            encontrado = encontrado || frag.equals(f);
        }
        if (encontrado)return;
        
        // actualiza lista fragmentos
        _temporales.get(f._hash).getFragmentos().add(f);
        // TODO: insertar en el GestorCompartidos el nuevo archivo si es el ultimo fragmento y eliminar de esta tabla hash
